...
import {
  appendPublishedWordHistory,
  buildWordDecision,
  loadWordAutopilotHistory,
  saveWordAutopilotHistory,
} from "./runtime/word-autopilot-engine.mjs";
import {
  appendIssuedWordHistory,
  consumePreparedWord,
} from "./runtime/word-content-creator-engine.mjs";
...
const DEFAULT_OUTPUT_ROOT = path.join(repoRoot, "word-autopilot-runs");
const DEFAULT_CREATOR_OUTPUT_ROOT = path.join(controlCenterRoot, "runtime", "queues", "word-content-creator");
...
const DEFAULT_PREPARED_QUEUE_PATH = path.join(
  ...
  "latest",
  "prepared_word_queue.latest.json",
);

enableStrictNonInteractiveMode("run-word-autopilot-cycle");
...
function printHelp() {
  console.log(`Usage: node scripts/run-word-autopilot-cycle.mjs [options]

Options:
  --prepared-queue <file> Queue JSON from word content creator (default: ${DEFAULT_PREPARED_QUEUE_PATH})
  --issued-history <file> Issued word history JSON (default: ${DEFAULT_ISSUED_HISTORY_PATH})
  ...
  --recent-limit <n>     Avoid the most recent N published words when possible (default: 3)
...
async function main() {
  ...
  const creatorOutput = runNodeScript(
    path.join(repoRoot, "scripts", "run-word-content-creator.mjs"),
    [
  ...
  const preparedQueuePath = parseOutputPath(creatorOutput, "latest_prepared_word_queue") || args.preparedQueuePath;
  const preparedQueue = await loadJson(preparedQueuePath, { entries: [] });
  const historyEntries = await loadWordAutopilotHistory(args.historyPath);
  ...
  const consumed = consumePreparedWord(preparedQueue);
  ...
  const selected = selectedEntry
    ...
    : null;
  const traceId = `word-autopilot-${Date.now()}-${text(selected?.source_id || "blocked")}`;
  const decision = buildWordDecision(selected, selectionMeta, traceId);
  ...
  logAutonomousDecision("word autopilot selected source", {
    source_id: decision.selected_source_id,
    fallback_mode: selectionMeta.fallback_mode || "",
    recent_limit: args.recentLimit,
  });

  const nextIssuedHistory = appendIssuedWordHistory(issuedHistory, selectedEntry, {
    issued_at: startedAt,
    trace_id: traceId,
  });
  ...
  if (!args.skipPublish) {
    const nextHistory = appendPublishedWordHistory(historyEntries, selected, {
    ...
    await saveWordAutopilotHistory(args.historyPath, nextHistory);
  }
  ...
  const reportPath = path.join(outputDir, "word_autopilot_report.json");
  await writeJson(reportPath, report);

  console.log(`output_dir=${outputDir}`);
  console.log(`decision=${decisionPath}`);
  console.log(`dispatch_report=${dispatchReportPath}`);
  console.log(`word_autopilot_report=${reportPath}`);
