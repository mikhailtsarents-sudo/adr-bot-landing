import { loadWordCatalog } from "./word-autopilot-engine.mjs";
...
export async function buildPreparedWordQueue(wordDir, options = {}) {
  ...
  const catalog = await loadWordCatalog(wordDir);
  ...
  for (let index = 0; index < horizon; index += 1) {
    ...
    const queueEntry = {
      queue_entry_id: `word-queue-${index + 1}-${selected.source_id}`,
      slot_index: index + 1,
      source_type: "WORD",
...
export function consumePreparedWord(queuePayload) {
...
export function appendIssuedWordHistory(issuedHistory, queueEntry, metadata = {}) {
  return [
