import { loadNewsCatalog } from "./news-autopilot-engine.mjs";
...
export async function buildPreparedNewsQueue(options = {}) {
  ...
  const catalog = await loadNewsCatalog(options);
  ...
  for (let index = 0; index < horizon; index += 1) {
    ...
    const queueEntry = {
      queue_entry_id: `news-queue-${index + 1}-${selected.source_id}`,
      slot_index: index + 1,
      source_type: "NEWS",
...
export function consumePreparedNews(queuePayload) {
...
export function appendIssuedNewsHistory(issuedHistory, queueEntry, metadata = {}) {
  return [
    ...(Array.isArray(issuedHistory) ? issuedHistory : []),
    {
      source_id: text(queueEntry?.source_id),
      draft_id: text(queueEntry?.source_payload?.draft_id),
      row_id: text(queueEntry?.source_payload?.id),
